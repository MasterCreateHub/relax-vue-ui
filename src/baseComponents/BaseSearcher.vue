<template>
    <el-card :body-style="{ paddingBottom: '0px' }">
        <el-form class="base-searcher" :model="form" label-position="right" v-bind="formConfig" @keyup.enter.native="handleSearch">
            <el-row :gutter="gutter">
                <el-col v-for="factor in showFactors" :key="factor.prop" :span="span">
                    <el-form-item :prop="factor.prop" :label="factor.label">
                        <component :is="factor.type || 'el-input'" v-model="form[factor.prop]"
                            clearable v-bind="factor.config" class="base-factor-item">
                ...
                <el-col v-if="$scopedSlots.action" class="base-searcher-actions" :span="span">
