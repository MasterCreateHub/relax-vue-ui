<template>
  <div v-measure="onResize" class="re-searcher">
    <el-form
      :class="[{ 'is-justify': labelPosition === 'justify' }]"
      :model="form"
    ...
    >
      <el-row class="re-searcher__body" :gutter="factorSpacing">
        <el-col
          class="re-searcher-factor__wrapper"
        ...
        <el-col class="re-searcher-action__wrapper" :span="factorSpan">
          <el-form-item>
...
export default {
  ...
  computed: {
    ...
    showFactors() {
      if (!this.expanded) {
        let sizeSpan = this.sizes.find(
          (size) => size.label === this.viewportSize
        ).value;
        let inlineMaxNum = (24 / sizeSpan - 1) ? (24 / sizeSpan - 1) : 1;
    ...
    onlyInline() {
      ...
      let inlineMaxNum = 24 / sizeSpan - 1;
     ...
     */
    factorSpan() {
      return this.sizes.find((size) => size.label === this.viewportSize).value;
    },
  },
  mounted() {
    this.onInit();
