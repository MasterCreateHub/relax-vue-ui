<template>
  ...
  export default {
    ...
    props: {
       ...
       * @description 搜索表单配置
       * @type {Object}
       * @default {size: "small", labelWidth: "60px"}
       */
      formConfig: {
        type: Object,
        default: () => {
       ...
       * @description 搜索条件数组
       * @type {Array}
       * @property  {Object} [] 搜索组件的配置项
       * @property  {String} [].label 搜索条件名称
       * @property  {String} [].prop 搜索条件绑定的属性，即表单组件要绑定的v-model
       * @property  {String} [].type 搜索条件组件类型，默认为el-input
       * @property  {Object} [].config 搜索条件组件的配置项，默认为{}
       * @property  {Array} [].options 搜索条件组件的选项，默认为[]，一般用于el-select、el-radio-group、el-checkbox-group等组件
       * @property  {String} [].valueType 搜索条件组件绑定值的类型，默认为String
       * @default []
       */
      factors: {
        type: Array,
    ...
    data() {
      return {
         ...
         * @description size配置
         * @type {Array}
         * @default [{label: 'xs', value: 1}, {label: 'sm', value: 2}, {label: 'md', value: 3}, {label: 'lg', value: 4}, {label: 'xl', value: 6}]
         */
        sizes: [
          { label: "xs", value: 1 },
          { label: "sm", value: 2 },
    ...
    computed: {
      ...
      /**
       * @description 计算搜索条件是否只有一行
       * @returns {Boolean} false: 多行，true: 一行
       */
      onlyInline() {
        let sizeValue = this.sizes.find(
          (size) => size.label === this.viewportSize
